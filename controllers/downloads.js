...
exports.createDownloadLink = async function (req, res) {
  const { name, email, company, country, interest, pluginsSourceCode, newsletter, release } =
    req.body;
  ...
  try {
    ...
    await DB.models.download_request.create({
  ...
  try {
    let info = await transporter.sendMail({
      ...
      html: downloadTemplate.buildTemplate(
        name,
        'Dowload',
        `Dicoogle v${release.tag_name}`,
        `${process.env.APP_URL}/download/${hash}`
      ),
      attachments: [
        {
          filename: 'Logo.png',
...
exports.download = async (req, res) => {
  ...
  if (downloadRequest.downloadLink) {
    await DB.models.download_request.update(
      { count: downloadRequest.count + 1 },
      { where: { hash } }
    );
