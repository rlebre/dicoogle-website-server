...
export class DownloadRequestService {
  constructor(
    @InjectRepository(DownloadRequest)
    private readonly downloadRequestRepository: Repository<DownloadRequest>,
    @InjectRepository(User) private readonly userRepository: Repository<User>,
    private mailService: MailService
  ) {}

  async create(
    downloadRequest: CreateDownloadRequestDto & CreateUserDto
    ...
    if (downloadRequest.pluginsSourceCode) {
      const pluginDownloadRequest = this.downloadRequestRepository.create({
        resource: 'dicoogle-plugins-sources-v3.0.0'
      });
  ...
  async getLink(hash: string): Promise<string> {
    const downloadLink = await this.downloadRequestRepository.findOneBy({ hash });
    ...
    const redirectLink =
      DicoogleReleasesService.ghReleases[downloadLink.resource]?.downloadLink ||
